...
class LegalDocument(db.Model):
    ...
    doc_type = Column(String(50), nullable=False)  # 'case', 'statute', 'regulation', 'article', 'user_upload'
    ...
    # Relationships
    citations_made = relationship('Citation', foreign_keys='Citation.citing_doc_id', backref='citing_document')
    citations_received = relationship('Citation', foreign_keys='Citation.cited_doc_id', backref='cited_document')
    annotations = relationship('DocumentAnnotation', backref='document', cascade='all, delete-orphan')
...
class LegalLibraryService:
    ...
    def ingest_from_courtlistener(self, citation: str) -> Optional[LegalDocument]:
        ...
        try:
            ...
            doc = LegalDocument(
                ...
                decision_date=datetime.fromisoformat(result.get('dateFiled')) if result.get('dateFiled') else None,
    ...
    def get_related_cases(self, doc_id: int, limit: int = 10) -> List[LegalDocument]:
        ...
        related = LegalDocument.query.filter(
    ...
    def _extract_topics(self, text: str) -> List[str]:
        ...
        topic_keywords = {
            '4th Amendment': ['fourth amendment', 'search and seizure', 'warrant', 'probable cause'],

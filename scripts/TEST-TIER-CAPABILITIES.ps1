#!/usr/bin/env pwsh
# BarberX.info - Tier-Based Feature Testing
# Tests PDF and BWC processing for Free vs Enterprise users

Write-Host "`n?? BarberX Tier Testing - Free vs Enterprise" -ForegroundColor Cyan
...
with app.app_context():
    ...
    free_user = User.query.filter_by(email="free@barberx.test").first()
    if not free_user:
        free_user = User(
    ...
    enterprise_user = User.query.filter_by(email="enterprise@barberx.test").first()
    ...
    print(f"  Free User: free@barberx.test (ID: {free_user.id})")
    print(f"  Enterprise User: enterprise@barberx.test (ID: {enterprise_user.id})")
'@
